...
const PlayQuize = () => {
  ...
  try {
    useEffect(() => {
      if (quizes?.length &&  quizes?.length > dayliQuize) {
        if (dbCurrentQuestion <= dayliQuize) {
          const interval = setInterval(() => {
            if (remainingTime === 0) {
              ...
              if (dbCurrentQuestion !== dayliQuize) {
                try {
                  fetch(`https://online-quize-server.vercel.app/auto-submit-quize`, {
                    method: "PUT",
                    headers: {
                    ...
                    .then((result) => {
                      console.log(result);
                      if(result.message === "auto submit Quize Update"){
                        toast.error(`You Loos ${autosubmitPoint} point beacuse of timeout  `);
                      }
                    });
                } catch (err) {}
              }
  ...
  const submitQuize = (option) => {
    if (option === answer) {
      ...
      try {
        fetch(`https://online-quize-server.vercel.app/right-quize`, {
          method: "PUT",
          headers: {
            "content-type": "application/json",
          },
          ...
          .then((result) => {
            ...
            if(result.message === "right answer points Update"){
              toast.success(`Right Ans  ${icressPoint} point Add `);
            }
          });
      } catch (err) {}
    }
    if (option !== answer) {
      ...
      try {
        fetch(`https://online-quize-server.vercel.app/wrong-quize`, {
          method: "PUT",
          headers: {
            "content-type": "application/json",
          },
          ...
          .then((result) => {
            console.log(result);
            if(result.message === "wrong answer points Update"){
              toast.error(`wrong Ans  ${decressPoint} point loose`);
            }
          });
      } catch (err) {}
    }
  ...
  if (currentQuestion <= dayliQuize) {
    try {
      fetch(
        `https://online-quize-server.vercel.app/store-daily-quize?email=${user?.email}&date=${date}&categoryName=${categoryName}`
      )
        .then((res) => res.json())
        .then((data) => {
      ...
      console.log("Faield store-daily-quize data");
    }
  }
        ...
        <>
          <div className="md:w-6/12 lg:w-6/12 w-10/12 mx-auto text-center mt-11">
            <div className="flex justify-center items-center text-2xl">
              {" "}
              {currentQuestion}/{dayliQuize} Quize Left{" "}
            </div>
            ...
            <h2 className="text-xl text-center">
              {quizes[dbCurrentQuestion]?.title}{" "}
            </h2>
